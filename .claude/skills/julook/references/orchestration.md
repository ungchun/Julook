...
Task #2: Phase 2 — GREEN 최소 구현 (owner: tca-implementer, blockedBy: [#1])
Task #3: Phase 3 — REFACTOR (owner: tca-implementer, blockedBy: [#2])
...
From: qa-verifier
To: tdd-orchestrator
Content: "Phase 4 실패: xcodebuild 빌드 에러. {파일:줄} {에러 요약}. 담당 Phase 식별 필요."
```
...
3. tca-implementer를 Phase 3부터 재호출
4. Phase 4~5 연쇄 재검증
...
2. 마지막+1 Phase부터 재개
3. 만약 Phase 3에서 멈춤 → tca-implementer에게 "Phase 3 이어서" 지시
...
| Phase 2 GREEN 실패 | tca-implementer → tdd-orchestrator → (최소 코드 축소 재시도) |
| Phase 3 리팩토링으로 기능 깨짐 | qa-verifier → tca-implementer → (`git restore` 후 더 작게 쪼개기) |
| Phase 4 빌드/테스트 실패 | qa-verifier → tdd-orchestrator → (원인 Phase 식별 후 담당자 반환) |
| Phase 5 pre-push 체크 실패 | qa-verifier → 해당 에이전트 (`print` → tca-implementer, 테스트 누락 → test-author) |
...
3. Phase 4 빌드 환경 문제 (mise/tuist 미설치 등)
